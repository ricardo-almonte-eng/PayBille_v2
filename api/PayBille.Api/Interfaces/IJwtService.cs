...
public interface IJwtService
{
    ...
    /// </summary>
    Task<Res_UserAuth_Dto> LoginAsync(string username, string password, CancellationToken ct = default);

    /// <summary>
    /// Validates the supplied refresh token, revokes it, issues a new pair,
    /// and persists the new refresh token.
    /// </summary>
    Task<Res_UserAuth_Dto> RefreshAsync(string refreshToken, CancellationToken ct = default);
